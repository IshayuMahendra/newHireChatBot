...
router.patch('/tasks/:id', authenticateToken, asyncHandler('PATCH /tasks/:id', async (req, res) => {
    const taskId = Number(req.params.id);
    if (!Number.isInteger(taskId) || taskId < 1) {
        return res.status(400).json({ error: 'Invalid task ID' });
    }
    ...
    if (typeof text !== 'string' || text.trim() === '') {
        return res.status(400).json({ error: 'body must contain non-empty text' });
    }
    const ownerId = await taskRepository.getTaskOwnerId(taskId);
    if (ownerId === null) {
        return res.status(404).json({ error: 'Task not found' });
    ...
    const updatedTask = await taskRepository.updateTaskText(taskId, text);
    if (!updatedTask) {
        return res.status(404).json({ error: 'Task not found' });
    }
    return res.status(200).json(updatedTask);
